...
def load_mesh_geometry(surf_mesh):
    ...
    if isinstance(surf_mesh, str):
        if (surf_mesh.endswith('orig') or surf_mesh.endswith('pial') or
            ...
            coords, faces, n_vert, norms = read_obj(surf_mesh)
            return {'coords':coords,'faces':faces, 'n_vert': n_vert, 'normals': norms}
...
def load_mesh_data(surf_data, gii_darray=0):
    ...
    if isinstance(surf_data, str):
        ...
        elif surf_data.endswith('.mgh'):
            data = load_mgh(surf_data)
        else:
